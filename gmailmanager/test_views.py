...
from gmailmanager.settings import gmail_settings
...
class SetupViewTestCase(TestCase):
    ...
    def test_setup_redirects_anon_user(self):
        ...
        response = SetupEmailAuthView.as_view()(request)
...
class CallbackViewTestCase(TestCase):
    ...
    def test_callback_redirects_anon_user(self):
        ...
        response = OAuth2CallbackView.as_view()(request)
        self.assertEqual(response.status_code, 302)
        self.assertIn(reverse('login'), response.url)
    ...
    @patch.object(OAuth2CallbackView, 'validate_token', return_value=False)
    def test_callback_checks_token(self, mock_method):
        request = self.factory.get('%s?state=testing_state&code=testing_code' % reverse('gmail_callback'))
        request.user = self.user
        OAuth2CallbackView.as_view()(request)
    ...
    @patch.object(OAuth2CallbackView, 'validate_token', return_value=True)
    def test_callback_correct_token_gets_credentials(self, mock_validation):
        ...
        with patch.object(OAuth2CallbackView, 'get_credentials') as mock_method:
            with patch('gmailmanager.views.EmailAccount'):
                OAuth2CallbackView.as_view()(request)
    ...
    def test_callback_correct_credentials_creates_account(self, mock_validation):
        ...
        with patch.object(OAuth2CallbackView, 'get_credentials', return_value=mock_credentials):
            with patch('gmailmanager.views.EmailAccount') as email_account:
                OAuth2CallbackView.as_view()(request)
